...
function ScenarioCard({ questions, chapterId }) {
  ...
  return (
    <div className="border-2 rounded-2xl overflow-hidden" style={{ borderColor: open ? 'rgba(34,197,94,0.6)' : 'rgba(255,255,255,0.85)', transition: 'border-color 0.2s' }}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full text-left px-4 py-3 flex items-center gap-3 transition-colors"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs mt-0.5 leading-snug" style={{ color: open ? '#4ade80' : '#64748b' }}>
            Real situations, real decisions. No lock — open to all levels.
          </div>
        </div>
