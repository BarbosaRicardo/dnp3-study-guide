...
function exportCSV(submissions) {
  ...
  a.href = url; a.download = `dnp3_quiz_report_${Date.now()}.csv`; a.click()
  URL.revokeObjectURL(url)
}

const AMBER = 'rgba(245,158,11,0.7)'
...
export default function QuizReport({ onClose }) {
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-y-0 right-0 w-full max-w-2xl z-50 flex flex-col shadow-2xl overflow-hidden"
        style={{ background: 'rgba(8,13,16,0.98)', border: '1px solid rgba(245,158,11,0.15)' }}
      >
        {/* Header */}
        <div
          className="flex items-center justify-between px-5 py-4 flex-shrink-0"
          style={{ borderBottom: '1px solid rgba(245,158,11,0.12)', background: 'rgba(0,0,0,0.3)' }}
        >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={refresh}
              className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:text-amber-400 transition-colors"
        ...
        {!loading && total > 0 && (
          ...
          >
            {[
              { label: 'Total', value: total, color: '#fbbf24' },
              ...
              <div key={label} className="px-4 py-3 text-center" style={{ background: 'rgba(8,13,16,0.9)' }}>
        ...
        {!loading && total > 0 && (
          <div
            className="flex gap-4 px-5 py-2.5 flex-shrink-0"
            style={{ borderBottom: '1px solid rgba(245,158,11,0.07)', background: 'rgba(0,0,0,0.2)' }}
          >
            {byLevel.map(({ level, count }) => (
              <div key={level} className="flex items-center gap-1.5">
                <span
                  ...
                  style={{
                    background: level === 1 ? 'rgba(34,211,238,0.12)' : level === 2 ? 'rgba(245,158,11,0.12)' : 'rgba(239,68,68,0.12)',
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center h-full gap-2 text-slate-600">
              <Loader2 size={18} className="animate-spin" style={{ color: '#fbbf24' }} />
              <span className="text-sm">Fetching from Supabase…</span>
            </div>
          ) : total === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              <div className="text-4xl mb-3">📊</div>
              <div className="font-bold text-slate-400 mb-1">No quiz submissions yet</div>
              <div className="text-sm text-slate-600">Complete quizzes in any chapter to see your history here.</div>
            </div>
          ) : (
            <table className="w-full text-xs">
              <thead>
                <tr style={{ background: 'rgba(245,158,11,0.06)', borderBottom: '1px solid rgba(245,158,11,0.12)' }}>
                  {['Date', 'Chapter', 'Level', 'Score', 'Result'].map(h => (
                    <th key={h} className="px-4 py-2.5 text-left font-bold text-slate-500 uppercase tracking-widest">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {submissions.map((s, i) => (
                  <tr
                    key={s.id || i}
                    style={{ borderBottom: '1px solid rgba(245,158,11,0.06)', background: i % 2 === 0 ? 'transparent' : 'rgba(245,158,11,0.02)' }}
                  >
                    <td className="px-4 py-2.5 text-slate-500 font-mono whitespace-nowrap">{fmtDate(s.created_at || s.ts)}</td>
                    <td className="px-4 py-2.5 text-slate-300">{chapterLabel(s.chapter)}</td>
                    <td className="px-4 py-2.5">
                      <span
                        className="px-1.5 py-0.5 rounded font-bold"
                        style={{
                          background: s.level === 1 ? 'rgba(34,211,238,0.1)' : s.level === 2 ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)',
                          color: s.level === 1 ? '#22d3ee' : s.level === 2 ? '#fbbf24' : '#f87171',
                        }}
                      >{LEVEL_LABEL[s.level]}</span>
                    </td>
                    <td className="px-4 py-2.5 font-mono text-slate-300">{s.score}/{s.total} <span style={{ color: s.pct >= 70 ? '#34d399' : '#f87171' }}>({s.pct}%)</span></td>
                    <td className="px-4 py-2.5">
                      {s.passed
                        ? <span className="flex items-center gap-1 text-green-400 font-bold"><CheckCircle2 size={12} /> Pass</span>
                        : <span className="flex items-center gap-1 text-red-400 font-bold"><XCircle size={12} /> Fail</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {total > 0 && (
          <div
            ...
            style={{ borderTop: '1px solid rgba(245,158,11,0.1)', background: 'rgba(0,0,0,0.2)' }}
          >
            <button
            ...
            {confirmClear ? (
              <div className="flex items-center gap-2">
                <span className="text-xs text-red-400">Clear local cache?</span>
                <button onClick={() => { clearLocal(); setConfirmClear(false) }} className="text-xs text-red-400 font-bold hover:text-red-300">Yes</button>
