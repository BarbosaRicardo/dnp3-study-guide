...
export default function Layers() {
  return (
    <ChapterLayout chapterId="layers" title="Protocol Layers" emoji="🧱" prev="intro" next="datalink">
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">The DNP3 Pseudo-OSI Stack</h2>
        <p>
      ...
      {/* Layer diagram */}
      <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden my-6">
        <div className="bg-navy-700 text-white text-xs font-bold px-4 py-2 uppercase tracking-widest">DNP3 Protocol Stack</div>
        <div className="p-4 space-y-2">
          {[
            { layer: 'Application Layer (Layer 4)', abbr: 'APDU', color: 'bg-mblue-600', desc: 'Function codes, data objects, IIN bits, fragmentation' },
            ...
            <div key={l.layer} className="flex items-center gap-4 bg-white rounded-xl border border-slate-100 p-3">
              <div className={`${l.color} text-white text-xs font-bold px-3 py-2 rounded-lg w-16 text-center flex-shrink-0`}>
                {l.abbr}
              </div>
              <div className="flex-1">
                <div className="font-semibold text-navy-700 text-sm">{l.layer}</div>
                <div className="text-xs text-slate-500 mt-0.5">{l.desc}</div>
              </div>
            </div>
          ))}
        </div>
        <div className="bg-slate-100 px-4 py-2 text-xs text-slate-500 text-center">
      ...
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">Physical Layer</h2>
        <p>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            ...
            <div key={m.medium} className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="font-semibold text-mblue-600">{m.medium}</div>
              <div className="text-xs text-slate-500 mt-1">Port / Interface: <span className="font-mono">{m.port}</span></div>
      ...
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">Transport Layer — The Forgotten Layer</h2>
        <p>
        ...
        <div className="mt-4 bg-slate-50 rounded-xl border border-slate-200 p-4 font-mono text-sm">
          ...
          <div className="space-y-2">
            <div className="flex gap-3"><span className="text-mblue-600 font-bold">FIR</span><span className="text-slate-700">= First fragment (bit 6) — this is the first transport segment</span></div>
            <div className="flex gap-3"><span className="text-mblue-600 font-bold">FIN</span><span className="text-slate-700">= Final fragment (bit 7) — this is the last transport segment</span></div>
            <div className="flex gap-3"><span className="text-mblue-600 font-bold">Seq</span><span className="text-slate-700">= Sequence number (bits 0-5) — 0 to 63, wraps around</span></div>
          </div>
