...
export default function FunctionCodes() {
  return (
    <ChapterLayout chapterId="fc" title="Function Codes" emoji="⚙️" prev="objects" next="unsol">
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">Function Codes: The Verbs of DNP3</h2>
        <p>
          ...
          <tbody>
            {FUNCTION_CODES.map((row, i) => (
              <tr key={row.fc} className={i % 2 === 0 ? 'bg-slate-50' : 'bg-white'}>
                <td className="px-3 py-2 font-mono font-bold text-mblue-600 whitespace-nowrap">{row.fc}</td>
                <td className="px-3 py-2 font-semibold text-slate-700 whitespace-nowrap">{row.name}</td>
                <td className="px-3 py-2 text-slate-500 whitespace-nowrap text-xs">{row.dir}</td>
      ...
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">Select-Before-Operate (SBO)</h2>
        <p>
          ...
          <div className="flex flex-col md:flex-row items-start gap-4">
            <div className="flex-1 space-y-3">
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 bg-mblue-600 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0">1</div>
                <div>
                  <div className="font-semibold text-navy-700">Select (FC=0x03)</div>
                  <div className="text-xs text-slate-500 mt-1">Master sends: "I intend to open Breaker 3A." Outstation validates and responds with the same control block echoed back. If everything matches, outstation arms and starts a countdown timer (typically 5–30 seconds).</div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-navy-700">Operate (FC=0x04)</div>
                  <div className="text-xs text-slate-500 mt-1">Master sends: "Operate the control I just selected." Outstation verifies the operate matches the select, verifies the timer hasn't expired, and executes. If any check fails, the operate is rejected.</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section>
        <h2 className="text-xl font-bold text-navy-700 mb-3">Direct Operate vs SBO</h2>
        <p>
